...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO api_keys (customer_id, key_hash) VALUES ($1::uuid, $2) RETURNING id, created_at',
...
router.get('/:id', async (req, res) => {
  ...
  const result = await pool.query(
    'SELECT id, created_at, last_used, revoked FROM api_keys WHERE id = $1 AND customer_id = $2::uuid',
...
router.delete('/:id', async (req, res) => {
  ...
  const result = await pool.query(
    'UPDATE api_keys SET revoked = TRUE WHERE id = $1 AND customer_id = $2::uuid RETURNING id',
    [keyId, req.customer.id]
  );
